...
export type Results = { mission_id: string; stage: string; cleaned_data_summary: { original_rows?: number; cleaned_rows?: number; columns?: string[]; cleaning_log?: string[] }; charts_data: { top_categories: Record<string, Record<string, number>>; correlations: Record<string, Record<string, number>>; trend: { date_column?: string; metric?: string; points?: { period: string; value: number }[] }; distribution?: { metric?: string; buckets?: { range: string; count: number }[] } }; approved_insights: Insight[]; rejected_insights: { insight: Insight; reason: string }[] };
function apiBaseUrl(): string { if (!API_URL) throw new Error("NEXT_PUBLIC_API_URL is not configured."); return API_URL; }
async function request<T>(path: string, init?: RequestInit): Promise<T> { const res = await fetch(`${apiBaseUrl()}${path}`, init); if (!res.ok) throw new Error((await res.text()) || `Request failed (${res.status})`); return res.json(); }
